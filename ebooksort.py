...
# Configure the OpenAI API using the key from config.ini
try:
    api_key = config.openai['api_key']
    if not api_key or api_key == "YOUR_API_KEY_HERE":
        logger.error("OpenAI API key is not set in config.ini. Please add it to the [OpenAI] section.")
        sys.exit(1)
    client = OpenAI(api_key=api_key)
    logger.info("OpenAI API configured successfully.")
except (KeyError, AttributeError):
    logger.error("Failed to configure OpenAI API. Make sure the api_key is set in config.ini.")
    sys.exit(1)
...
def get_epub_title_from_openai(filename_stem, dry_run=False):
    """Extracts just the book title from an epub filename using OpenAI."""
    if dry_run:
        logger.info(f"DRY RUN: Would call OpenAI to extract title from: '{filename_stem}'")
        return sanitize_filename(filename_stem.title())
    try:
        response = client.chat.completions.create(
            model=config.openai['model_name'],
            messages=[
                {"role": "system", "content": "You extract book titles from filenames. Return ONLY the book title—no author, publisher, year, series, format, edition, or other metadata. Just the title, nothing else. "
                 "Use title case, but ALWAYS preserve well-known acronyms in uppercase: ACT, CBT, DBT, DSM, PTSD, EMDR, OCD, NLP, ADHD, WISC, SFBT, IFS, EFT, BPD, LGBTQ. "
                 "Never use colons, slashes, or other special characters that are invalid in filenames."},
                {"role": "user", "content": f"Extract only the book title from this filename: '{filename_stem}'"}
            ]
        )
        title = response.choices[0].message.content.strip().strip('"\'')
        title = sanitize_filename(title)
        logger.info(f"  - OpenAI extracted epub title: '{title}' from '{filename_stem}'")
        return title
    except Exception as e:
        logger.error(f"  - Error extracting epub title from OpenAI: {e}")
        return filename_stem.title()
    finally:
        time.sleep(config.openai['api_cooldown'])

def get_book_info_from_openai(info_string, dry_run=False):
    if dry_run:
        logger.info(f"DRY RUN: Would call OpenAI API for info string: '{info_string}'")
    ...
    prompt = config.openai['prompt'].format(info_string=info_string)
    try:
        response = client.chat.completions.create(
            model=config.openai['model_name'],
            messages=[
                {"role": "system", "content": "You are a helpful assistant that extracts book metadata from text. Always respond in the exact format requested."},
...
def pre_organize_into_folders(source_dir, staging_dir, dry_run=False):
    ...
    for item in items_to_process:
        ...
        # For epub files, extract just the book title via OpenAI
        if new_item_name.lower().endswith('.epub'):
            stem, ext = os.path.splitext(new_item_name)
            clean_title = get_epub_title_from_openai(stem, dry_run=dry_run)
            new_item_name = clean_title + ext.lower()
...
def organize_audio_files(base_dir, dest_dir, dry_run=False, no_tagging=False, force_openai=False):
    ...
    for root, _, files in os.walk(base_dir):
        ...
        try:
            ...
            # If not found on Google Books, or if the result is incomplete, or if OpenAI is forced, try OpenAI
            if force_openai or not book_data or book_data["title"] == "Unknown":
                if force_openai:
                    logger.info("Forcing OpenAI lookup as per user request.")
                else:
                    logger.info("Not found on Google Books or result was incomplete. Falling back to OpenAI.")

                logger.info(f"Using '{info_string}' to get book information from OpenAI...")
                book_data = get_book_info_from_openai(info_string, dry_run=dry_run)
            ...
            if not dry_run: time.sleep(config.openai['api_cooldown'])
...
def main():
    ...
    parser.add_argument("--force-openai", action="store_true", help="Force the use of OpenAI for fetching book data, skipping other sources.")
    ...
    try:
        ...
        books_without_cover = organize_audio_files(staging_dir, dest_dir_abs, dry_run=args.dry_run, no_tagging=args.no_tagging, force_openai=args.force_openai)
